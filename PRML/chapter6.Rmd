...
gp_samples <- function(X, dataset, kernel, n_samples, sd_noise=0, seed=101) {
...
plot(X, result$y_sample[,1], type="n", ylim=c(min(y_sample), max(y_sample)), ylab="sample functions")
for(i in 1:n_samples)
  points(X, result$y_sample[,i], type="l", col="lightgrey")
points(X, result$mean_est, type="l", col="red", lwd=2)
```

We can estimate the output for each value that defined the functions.

```{r}
# get more samples
result <- gp_samples(X, d, make_gaussian_kernel(0.5), n_samples=5e3)

i  <- 23 # eg: check the 23th value of X
ys <- result$y_sample[i,]
library(coda)
hpd <- HPDinterval(as.mcmc(ys), prob=0.95) # compute highest density interval

hist(ys, breaks=50, freq=FALSE, xlab="", main=paste("Estimate for x=",round(X[i],2)))
lines(density(ys),col="red",lwd=2)
text(2.1,1.5,paste("mean: ",round(mean(ys),2)))
text(2.0,2.0,paste("95% HPD: [",round(hpd[1],2),",",round(hpd[2],2),"]"))
```

Also, we can assume that our data is noisy:

```{r}
result <- gp_samples(X, d, make_gaussian_kernel(0.5), n_samples, sd_noise=0.1)

plot(X, result$y_sample[,1], type="n", ylim=c(min(y_sample), max(y_sample)),
...
# get more samples
result <- gp_samples(X, d, make_gaussian_kernel(0.5), n_samples=5e3, sd_noise=0.1)

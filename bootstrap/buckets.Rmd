...
Let's use two groups of data to check if their difference of means is significant:

```{r}
my.data <- list(group1 = c(27,20,21,26,27,31,24,21,20,19,23,24,28,19,24,29,18,20,
                           17,31,20,25,28,21,27),
                group2 = c(21,22,15,12,21,16,19,15,22,24,19,23,13,22,20,24,18,20))
...
diff.means <- function(indexes) {
  in.group1 <- 1:length(my.data$group1)
  indexes.group1 <- indexes[ in.group1]
  indexes.group2 <- indexes[-in.group1]
  # since our hypothesis is that they have the same mean, it is a two sided test
  # hence the abs()

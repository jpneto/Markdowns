...
This is a plot function for nnet neural nets:
...
raw.fun<-paste(
  root.url,
  '5086859/raw/71593216393fe43546e8cc4392d5838b8d35c7e1/nnet_plot_fun.r',
  sep='/'
  )
...
plot.nnet(nn.iris, pos.col='blue', neg.col='red', alpha.val=0.7, rel.rsc=4,
          circle.cex=8, cex=1.4, circle.col='gray')
```

And getting the NN's weights:

```{r}
plot.nnet(nn.iris,wts.only=T)
```

This plot function allows to filter the neurons that we want to present the connections using params all.in and all.out:

```{r}
plot.nnet(nn.iris, pos.col='blue', neg.col='red', alpha.val=0.7, rel.rsc=4,
          circle.cex=8, cex=1.4, circle.col='gray',all.in='Sepal W.',all.out='v')
```

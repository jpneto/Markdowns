...
check.income.NA <- function(for.race) {
  ...
  df <- sis_dp %>% filter(race==for.race, na.rm=TRUE)
  
  n    <- df %>% nrow()
  n.NA <- df %>% filter(is.na(earnings)) %>% nrow()
...
calc.mode <- function(xs) {
  us <- unique(xs)
  us[which.max(tabulate(match(xs, us)))]
}

means <- apply(as.matrix(sis_sm), 2, mean, na.rm=TRUE)
means
modes <- apply(sis_sm, 2, calc.mode)
modes
```
...
Imputations can result in strange values. In out dataset, the original income has no negative values:

```{r}
which(sis_sm$earnings<0)
```

but the imputed results could have had negative values (in this case, it has not):

```{r}
min(imp.data$imp$earnings)
```
-->
